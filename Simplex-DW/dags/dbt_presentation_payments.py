...
config = AwsManager.get_config(folder="transform_configs/",
                               file_name='presentation_payments_dbt_transforms',
                               file_type='yml')
extra_parameters = (Variable.get("dbt_time_delay", deserialize_json=True)).get('presentation_payments', '')
...
def run_transform():
    ...
    validation = PythonOperator(
        task_id=f'validation',
        python_callable=sfm.validation,
        op_kwargs={"config": config, 'notifier': error_notifier})
    ...
    for model in config.get('models'):
        ...
        if model.get('test'):
            ...
            log_started >> dbt_run_task >> dbt_test_task >> validation >> log_completed
        else:
            log_started >> dbt_run_task >> validation >> log_completed
